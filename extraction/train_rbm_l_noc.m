function model = train_rbm_l_noc(conf,dat,lab,testd,testl,log_file)
...
for e=1:conf.eNum;
    ...
    %if e>15,lr = conf.params(1)/ceil(e/5)^2; disp(lr); else, lr = conf.params(1)/ceil(e/5); end
    res_e = 0;
    acc_e = 0;
    spr_e = 0;        
    for b=1:conf.bNum
...
%        acc_e = acc_e + sum(sum(labP == labN))/sNum;
    ...
    [T R] = extract_rbm_l(model,'',0);
   
    Rs(1) = R;                    
    
    routput = rule_inference(Rs,T,testd);
